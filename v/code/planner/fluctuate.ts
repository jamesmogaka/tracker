...
export class fluctuate {
  ...
  private static separate_children(
    ...
    const elder: HTMLElement | undefined = fluctuate.get_elder(
      mother,
      children,
      dna
    );
    ...
    const all_siblings: Array<HTMLElement> = fluctuate.get_siblings(
      mother,
      children,
      dna
    );
  ...
  private static get_elder(
    ...
    const elders: Array<HTMLElement> = children.filter((child) =>
      child.contains(organ)
    );
    //
    //There must be one.
    if (elders.length == 0)
  ...
  private static contains(child: Element, organs: Array<HTMLElement>): boolean {
    ...
    const child_organs: Array<HTMLElement> = organs.filter((organ) =>
      child.contains(organ)
    );
    //
