...
//
//Get the dialog to help with data collection
import { dialog } from "../../../../mashamba/v/code/dialog.js";
//
//Get  the definition of dirty data from the quiz module
import { dirty } from "../../../../mashamba/v/loader/quiz.js";
//
...
export class presentation extends page {
  ...
  /*
      ...
      */
  private async show_presentation(
    ...
    this.create_element("h2", visual, {
      textContent: `MINUTES FOR ${surname.toUpperCase()} ON ${date}`,
    });
    //
    //Decode and display the projects under the presentation
    JSON.parse(projects).forEach((project: Iproject) =>
      this.show_project(project, visual)
    );
  }
  //
  ...
  private show_project(data: Iproject, visual: HTMLElement): void {
    ...
    const details: HTMLDetailsElement = this.create_element("details", visual, {
      className: "project",
    });
  ...
  private show_contributions(
    ...
    button.onclick = async () =>
      new contribution(anchor, this, minute_pk).edit();
    //
    //Display the individual contributions if any
    if (Icontributions)
      Icontributions.forEach((Icontribution) =>
        new contribution(
          anchor,
          this,
          Icontribution.minute,
          Icontribution
        ).display()
      );
  }
...
class contribution extends dialog<Icontribution> {
  ...
  constructor(
    ...
    super(anchor, true, "/tracker/v/code/mogaka/contribution.html");
  }
  //
  ...
  public async populate(): Promise<void> {
    //
    //Ensure that the native contribution is present for population to  take place
    if (!this.native) return;
    //
    //Extract the keys of the data gotten
    const keys: Array<keyof Icontribution> = Object.keys(this.native) as Array<
      keyof Icontribution
    >;
    //
    ...
    key_elements.forEach((key) => {
      ...
      const element: Array<any> = elements.filter(
      ...
      if (element.length === 0)
        throw new mutall_error(
          "Check your envelop labels and make sure they are correct"
        );
      //
      ...
      element[0].value = this.native![key];
    });
  }
  ...
  public display(): void {
    ...
    const anchor = this.create_element("details", this.anchor, {
      className: "contribution",
    });
    //
    //Show the summary of the contribution
    this.create_element("summary", anchor, {
      textContent: `${this.native?.summary} (${this.native?.surname})`,
    });
    //
  ...
  async read(): Promise<dirty<Icontribution>> {
    //
    ...
    if (summary === null)
      throw new mutall_error(
        "Summary should be marked as a required attribute"
      );
    //
  ...
  private async get_intern(user: number): Promise<number | Error> {
    ...
    if (results.length > 1)
      return new Error(
        `Database design problem!! Only 1 user is expected. ${results.length} found.`
      );
    //
  ...
  public async execute(contribution: Icontribution): Promise<"Ok" | Error> {
    //
  ...
  private *collect_layouts(data: Icontribution): Generator<quest.layout> {
    ...
    if (this.native) {
      ...
      yield [this.native.contribution!, "contribution", "contribution"],
        //
