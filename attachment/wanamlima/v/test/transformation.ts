...
export function get_coords(): void {
    ...
    if (!svg) throw new Error('No element with the given id exist in your document');
    //
...
function initialize(svg: SVGSVGElement): void {
    ...
    const areas: Array<HTMLAreaElement> = Array.from(document.body.querySelectorAll('area'));
    //
...
function transform_area(area: HTMLAreaElement, svg: SVGSVGElement): void {
    ...
    if (!str) throw new Error('The coords attribute was not supplied on the area element');
    //
    ...
    const [px, py]: [number, number] = transform_to_pixel(x, y, svg);
    //
    //Transform the radius using the real world coordinates to get the pixels
    const [r0, r1]: [number, number] = transform_to_pixel(0, r, svg);
    //
...
function transform_to_coords(
    ...
    //
    //Transform the point
    const { x, y }: { x: number; y: number } = point.matrixTransform(ctm?.inverse());
    //
    //X
    const px: number = Math.round(x);
    //
...
const values = transform_to_pixel(0, 0, document.querySelector('svg')!);

console.log(values);
console.log(transform_to_coords(values[0], values[1], document.querySelector('svg')!));
