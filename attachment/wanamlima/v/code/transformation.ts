...
function initialize(svg:SVGSVGElement): void {
    ...
    //
    console.log(transform_point(-49.4, 0, svg));
}
//          
//
function transform_area(area:HTMLAreaElement, svg:SVGSVGElement): void {
    ...
    const pr:number = r1 - r0;
    //
    //Extract the x and y 
    const result:string = `${px},${py},${pr}`;
    //
...
function transform_point(initial_x:number, initial_y:number, svg:SVGSVGElement): [x:number,y: number] {
    //
