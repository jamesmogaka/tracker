...
export class presentation extends page {
    ...
    async init():Promise<HTMLDivElement>{
        ...
        //
        //Create the element for holing the presentation visual
    ...
    private async show_presentation(presentation: Ipresentation): Promise<void> {
        ...
        this.create_element("h2", this.visual, {textContent: `MINUTES FOR ${surname.toUpperCase()} ON ${date}`});
        //
        //Decode and display the projects under the presentation
        JSON.parse(projects).forEach((project: Iproject) => this.show_project(project));
    ...
    /*<$anchor>
      ...
      */
    private show_project(data: Iproject): void {
        //
        //Destructure the project
        const {name, minutes} = data;
        //
        //Show the project tag that envelops the summary and the minutes
        const details: HTMLDetailsElement = this.create_element("details", this.visual!, {className: "project"});
    ...
    */
    ...
    private show_contributions(
        anchor: HTMLElement,
        contributions: Array<Icontribution> | null,
        minute_pk: number,
        owner: number,
    ): void {
        //
        ...
        if (contributions) contributions.forEach((contribution) => this.show_contribution(contribution, anchor));
    }
    //
    //Create a contribution
    private async create_contribution(
        //
        ...
        const result: Icontribution | "cancel" = await new Promise(
            async (resolve) => await this.get_and_save_contribution(resolve, minute_pk, owner)
        );
        //
        //If the contribution was saved successfully display the contribution
        if (result !== "cancel") this.show_contribution(result, anchor);
        //
    ...
    private async get_and_save_contribution(
        ...
        const contribution_form:string = await this.contribution_form();
        //
        //Create a dialog element 
        const dialog = this.create_element("dialog",this.visual!,{
            id: "contributions_dialog"
        });
        //
        //Recreate the contribution form within the dialog box
        dialog.innerHTML = contribution_form;
        //
        //Get the input elements and clear the input (incase its dirty)
        //
        //Clear the detail
        ...
        dialog.showModal();
        //
        //After entering contribution details the user can either
        //
        //...save the contriburion
    ...
    //
    //Collect the form that will be used to get user inputs.
    private async contribution_form(): Promise<string> {
        //
        // Get a server response using the presentation file
        const response: Response = await fetch(`/tracker/v/code/mogaka/contribution.html`);
        // 
    ...
    public async save_contribution(
        //
