...
with
 ...
 #
    # Extract the raw data for projects of an intern which include name, problem,
    # plan, outcome
    projects as(
        select
            workplan.intern,
    ...
    raw_2 as (
        ...
        from
            raw
            left join workplan on workplan.intern = raw.intern
    ...
    intern as(
        select
            ...
            json_object('intern',intern,'name',name,'surname',surname,'image',image,'qualification',qualification,
            'university',university,'year',year,'presenter',is_presenter,'presentations',presentations,'workplan',workplan) as intern
        from
            raw_2
        order by day_code
    ),
    ...
    day as(
        select
            day_code,
            day_name,
            json_arrayagg(intern) as intern
        from
            intern
